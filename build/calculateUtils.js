...
/*: Position*/
{
  ...
  if (state && state.resizing) {
    ...
    out.height = Math.round(state.resizing.height);
      ...
      if (lockedRatio) {
        var relativeRowHeight = colWidth / lockedRatio;
        var relativeColumnWidth = relativeRowHeight * lockedRatio;
  ...
  if (state && state.dragging) {
    ...
    out.left = Math.round(state.dragging.left);
      ...
      if (lockedRatio) {
        var _relativeRowHeight = colWidth / lockedRatio;

        var _relativeColumnWidth = _relativeRowHeight * lockedRatio;

        out.left = Math.round((_relativeColumnWidth + margin[0]) * x + containerPadding[0]);
        out.top = Math.round((_relativeRowHeight + margin[1]) * y + containerPadding[1]);
      } else {
