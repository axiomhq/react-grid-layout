...
var GridItem = (function(_React$Component) {
  ...
  GridItem.prototype.calcPosition = function calcPosition(x, y, w, h, state) {
    var _props2 = this.props,
      ...
      lockedRatio = _props2.lockedRatio;
    ...
    if (lockedRatio) {
      ...
      out.left = Math.round(
        (relativeColumnWidth + margin[0]) * x + containerPadding[0]
      );
      out.top = Math.round(
        (relativeRowHeight + margin[1]) * y + containerPadding[1]
      );
      out.width =
        w === Infinity
          ? w
          : Math.round(
              relativeColumnWidth * w + Math.max(0, w - 1) * margin[0]
            );
      out.height =
        h === Infinity
          ? h
          : Math.round(relativeRowHeight * h + Math.max(0, h - 1) * margin[1]);
    } else {
